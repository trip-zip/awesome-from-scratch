...
local function create_toggle(icon, label, key, on_toggle, check_cmd)
    ...
    container:connect_signal("mouse::enter", function()
        container.bg = toggle_states[key]
            and (beautiful.primary_color_hover or "#fe8019")
...
function toggles.create()
    -- WiFi toggle
    local wifi_toggle = create_toggle(
        "󰤨",
        "WiFi",
        "wifi",
        function(state)
        ...
        "nmcli radio wifi 2>/dev/null || echo off"
    )
    ...
    local dnd_toggle = create_toggle(
        "󰂛",
        "DND",
        "dnd",
        function(state)
    ...
    -- Night Light toggle
    local nightlight_toggle = create_toggle(
        "󰖔",
        "Night",
        "nightlight",
        function(state)
        ...
        "pgrep gammastep >/dev/null && echo on || echo off"
    )

    -- Airplane mode toggle
    local airplane_toggle = create_toggle(
        "󰀝",
        "Airplane",
        "airplane",
        function(state)
    ...
    -- Microphone toggle
    local mic_toggle = create_toggle(
        "󰍬",
        "Mic",
        "mic",
        function(state)
        ...
        [[wpctl get-volume @DEFAULT_AUDIO_SOURCE@ 2>/dev/null | grep -q MUTED && echo off || echo on]]
    )
