...
local function get_battery_icon(percent)
    if percent >= 90 then return "󰁹"
    elseif percent >= 80 then return "󰂂"
    elseif percent >= 70 then return "󰂁"
    elseif percent >= 60 then return "󰂀"
    elseif percent >= 50 then return "󰁿"
    elseif percent >= 40 then return "󰁾"
    elseif percent >= 30 then return "󰁽"
    elseif percent >= 20 then return "󰁼"
    elseif percent >= 10 then return "󰁻"
    else return "󰁺"
    end
end
...
local function update_battery()
    if not battery_widget then return end
...
function M.init()
    ...
    local function update_caps_lock()
        awful.spawn.easy_async_with_shell([[
            ...
            if caps_on then
                caps_warning.markup = string.format(
                    '<span foreground="%s">⚠ Caps Lock is ON</span>',
                    colors.soft_orange
                )
            else
                caps_warning.markup = ""
